...
const SCORM_API_SIMULATOR = {
  setValue: (element, value) =>
    console.log(`SCORM API: Setting ${element} to "${value}"`),
  getValue: (element) => {
    const data = {
...
const ScormPlayer = ({ coursePath, courseId, title }) => {
  ...
  const handleIframeLoad = () =>
    SCORM_API_SIMULATOR.setValue("cmi.core.entry", "resume");
  const handleError = () =>
    setErrorMessage(`Error loading "${title}". Check path: ${coursePath}`);
  ...
  return (
    <div className="flex flex-col h-full bg-gray-50 dark:bg-gray-900">
      <div className="p-3 border-b border-gray-200 bg-white shadow-sm flex justify-between items-center dark:bg-gray-800 dark:border-gray-700">
        <h2 className="text-xl font-semibold text-gray-700 truncate dark:text-white">
          {title}
        </h2>
  ...
  return (
    <div className="relative">
      <ThemeProvider theme={muiTheme}>
        ...
        <Router>
          <div
            className={`flex relative h-screen font-sans ${
