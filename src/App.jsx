...
const App = () => {
  ...
  // 'Default', 'Slim', 'Stacked'

  const [sidenavShape, setSidenavShape] = useState('Default');
  ...
  // --- Synchronization Logic: Sidenav Shape <-> Collapsed State ---

  useEffect(() => {

    if (layout === 'Sidenav' || layout === 'Combo') {

        // 'Slim' corresponds to the collapsed state

        if (sidenavShape === 'Slim') {
  ...
  // Function to allow manual toggling of collapse while respecting the Sidenav Shape setting

  const handleSidebarToggle = () => {

    // If the user manually toggles collapse, we override the current sidenavShape state
    ...
    // Update sidenavShape to reflect the manual toggle (Default/Slim)

    if (newCollapsedState) {

      setSidenavShape('Slim');

    } else if (sidenavShape === 'Slim') {

      setSidenavShape('Default');

    }

    // Note: Stacked is not managed by the simple toggle, keeping it simple.
          ...
          <div className={`flex relative h-screen font-sans ${theme === 'dark' ? 'bg-slate-900' : 'bg-white'}`}>
            ...
            {shouldRenderSidebar && (
              ...
              <Sidebar
                ...
                // Use the new handler for toggling
                ...
                sidenavShape={sidenavShape} // Pass down the current shape
            ...
            <div className="flex flex-col flex-1 overflow-hidden">
              ...
              <CustomizeDrawer
                ...
                // 👇 NEW PROPS for Sidenav Shape

                sidenavShape={sidenavShape}

                setSidenavShape={setSidenavShape}
